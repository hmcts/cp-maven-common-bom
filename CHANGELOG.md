...
- `wildfly.version`: `26.1.2.Final` → `32.0.1.Final`
- `artemis.jms.version`: `2.24.0` → `2.32.0`
- `weld.version`: `3.1.4.Final` → `5.1.2.Final` (Weld 5, CDI 4.0 — required for WildFly 32)
- `hibernate.version`: `5.4.24.Final` → `6.6.1.Final` (Hibernate ORM 6; group ID changed to `org.hibernate.orm`)
- `resteasy.version` / `resteasy-client.version`: `3.15.5.Final` → `6.2.9.Final` (RESTEasy 6 — Jakarta EE 10 namespace)
- `byte-buddy.version`: `1.12.22` → `1.14.11`
- `jacoco-maven-plugin.version`: `0.8.8` → `0.8.12`
- `mockito.version`: `4.11.0` → `5.3.1`
- `openejb.version`: `8.0.13` → `10.1.4` (Jakarta EE 10 / CDI 4.0 compatible)
- Added new explicit Jakarta EE 10 dependencies: `jakarta.jms-api`, `jakarta.activation-api`, `jakarta.inject-api`, `artemis-jakarta-client`, `jackson-datatype-jakarta-jsonp`, `parsson`

### Fixed
- Corrected `jakarta.enterprise:jakarta.cdi-api` artifact ID to `jakarta.enterprise:jakarta.enterprise.cdi-api`

### Removed
- Removed legacy javax managed dependency entries that have no place in a Jakarta EE 10 BOM:
  `javax.xml.bind:jaxb-api`, `org.glassfish:javax.json`, `com.sun.mail:javax.mail`,
  `com.sun.xml.bind:jaxb-core`, `com.sun.xml.bind:jaxb-impl`

### Security
- Updated `json-smart` to `2.5.2` to fix **CVE-2023-1370** (Uncontrolled Recursion — denial of service via deeply nested JSON; CVSS 7.5)
- Updated `wiremock` to `3.x` to fix **CVE-2023-41329** (DNS rebinding attack on WireMock admin API; CVSS High)
- Updated `plexus-utils` to fix **CVE-2022-4244** (Path traversal) and **CVE-2022-4245** (XXE injection)
- Updated `jackson-databind` to fix **CVE-2022-42003** and **CVE-2022-42004** (Denial of service via JSON deserialization)
